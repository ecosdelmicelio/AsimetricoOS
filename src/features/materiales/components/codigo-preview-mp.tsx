...
import { generarCodigoMP, recomendarNombre, esCodigoCompleto } from '@/shared/lib/code-generator'
...
export function CodigoPreviewMP({
  ...
  const preview = useMemo(() => {
    ...
    const atributoSubtipo = atributos.find(a => a.id === subtipoId)
    ...
    const nombreRecomendado = recomendarNombre([atributoTipo, atributoSubtipo, atributoColor, atributoDiseño])

    const tieneAbreviacionTipo = !atributoTipo || !!atributoTipo.abreviacion
    const tieneAbreviacionSubtipo = !atributoSubtipo || !!atributoSubtipo.abreviacion
  ...
  }, [tipoId, subtipoId, colorId, disenoId, atributos])
  ...
  return (
    <div className="space-y-3">
      ...
      {!preview.tieneAbreviacionSubtipo && preview.atributoSubtipo && (
        <div className="rounded-xl bg-yellow-50 border border-yellow-200 p-3 flex gap-2">
          <AlertTriangle className="w-4 h-4 text-yellow-700 flex-shrink-0 mt-0.5" />
          <p className="text-xs text-yellow-700">
            El subtipo <span className="font-semibold">"{preview.atributoSubtipo.valor}"</span> no tiene abreviatura configurada
          </p>
        </div>
      )}
