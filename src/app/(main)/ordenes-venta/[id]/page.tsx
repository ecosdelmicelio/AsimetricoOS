...
import { OVDetail } from '@/features/ordenes-venta/components/ov-detail'

interface Props {
  params: Promise<{ id: string }>
}

export default async function OVDetailPage({ params }: Props) {
  const { id } = await params

  return (
    <div className="max-w-3xl mx-auto">
      <Suspense fallback={<OVDetailSkeleton />}>
        <OVDetail id={id} />
      </Suspense>
    </div>
  )
